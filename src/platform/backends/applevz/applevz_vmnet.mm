...
void start_vmnet_interface(VmnetRelay& relay, const std::string& physical_iface)
{
    relay.queue = dispatch_queue_create(
        fmt::format("com.elemento.elpd.vmnet.{}", physical_iface).c_str(),
...
{
    relay.queue = dispatch_queue_create(
        fmt::format("com.elemento.elpd.vmnet.zone.{}", subnet.to_cidr()).c_str(),
        DISPATCH_QUEUE_SERIAL);
...
void start_forwarding_from_vm(VmnetRelay& relay)
{
    ...
    relay.vm_queue =
        dispatch_queue_create("com.elemento.elpd.vmnet.vm", DISPATCH_QUEUE_SERIAL);
