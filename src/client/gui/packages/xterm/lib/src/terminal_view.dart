...
class TerminalViewState extends State<TerminalView> {
  ...
  /// Scroll controller for the terminal viewport (scrollback).
  ...
  void _onTapDown(TapDownDetails details) {
    // Shift+click extends the existing selection (or from the cursor) to the
    // clicked cell — standard terminal behavior for selecting across scrollback.
    if (HardwareKeyboard.instance.isShiftPressed) {
      final buffer = widget.terminal.buffer;
      final extent = renderTerminal.getCellOffset(details.localPosition);
      final base = _controller.selectionBaseOffset ??
          CellOffset(buffer.cursorX, buffer.absoluteCursorY);
