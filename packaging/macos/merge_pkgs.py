...
with tempfile.TemporaryDirectory() as workdir:
    ...
    elp_gui_component = "elp_gui"

    for path in x86_work.glob("*.pkg/Payload.unpacked/**/*"):
        if path.is_dir() or elp_gui_component in str(path.absolute()):
            continue
    ...
    pkgs.mkdir()
    for component in ("elp", "elpd", "elp_api"):
        pkg = dest_work / f"elp-{version}-Darwin-{component}.pkg"
        subprocess.check_call(["pkgbuild", "--root", pkg / "Payload.unpacked",
                               "--identifier", f"com.elemento.elp.{component}",
                               "--scripts", pkg / "Scripts",
                               "--version", version,
                               "--install-location", "/",
                               pkgs / pkg.name])

    # Handle elp-gui seperately since we need to pass in a component plist
    pkg = dest_work / f"elp-{version}-Darwin-{elp_gui_component}.pkg"
    ...
    subprocess.check_call(["plutil", "-replace", "BundleIsRelocatable",
                           "-bool", "false",
                            component_plist])
    subprocess.check_call(["pkgbuild", "--root", pkg / "Payload.unpacked",
                           "--identifier", f"com.elemento.elp.{elp_gui_component}",
                           "--scripts", pkg / "Scripts",
